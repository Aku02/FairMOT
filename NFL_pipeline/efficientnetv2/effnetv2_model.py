...
from . import effnetv2_configs, hparams, utils
...
def get_model(
  ...
  net(tf.keras.Input(shape=input_shape) if input_tensor is None else input_tensor,
      training=training,
  ...
  if pretrained is True: 
    pretrained_ckpt = tf.train.latest_checkpoint(model_name)
    net.load_weights(pretrained_ckpt)
